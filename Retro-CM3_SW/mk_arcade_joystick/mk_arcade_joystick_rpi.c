...
#define GPIO_READ(g)  *(gpio + ((g)/32) + 13) &= (1<<((g)%32))
...
static const int mk_arcade_gpio_maps[] = {42,39,40,43,45,44,35,36,38,37,34,41};
...
/* GPIO UTILS */
static void setGpioPullUps_1(int pullUps) {
    *(gpio + 37) = 0x02;
    udelay(10);
    *(gpio + 38) = pullUps;
    udelay(10);
...
static void setGpioPullUps_2(int pullUps) {
    *(gpio + 37) = 0x02;
    udelay(10);
    *(gpio + 39) = pullUps;
    udelay(10);
...
static int getPullUpMask_1(int gpioMap[]){
    int mask = 0x0000000;
    int i;
    for(i=0; i<12;i++) {
        if(gpioMap[i] != -1 && gpioMap[i] < 32){   // to avoid unused pins
...
static int getPullUpMask_2(int gpioMap[]){
    int mask = 0x0000000;
    int i;
    for(i=0; i<12;i++) {
        if(gpioMap[i] != -1 && gpioMap[i] > 31){   // to avoid unused pins
            int pin_mask  = 1<<(gpioMap[i]-32);
            mask = mask | pin_mask;
        }
    }
    return mask;
...
static int __init mk_setup_pad(struct mk *mk, int idx, int pad_type_arg) {
    ...
    if(pad_type != MK_ARCADE_MCP23017){
        ...
        setGpioPullUps_1(getPullUpMask_1(pad->gpio_maps));
        setGpioPullUps_2(getPullUpMask_2(pad->gpio_maps));
